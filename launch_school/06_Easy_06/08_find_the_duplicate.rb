...
APPROACH:
- we use a hash table to store elements we already encountered
- once we hit an element we already encountered, we return it
...
def find_dup(array)
  ...
  array.each do |element|
    return element if encountered[element]
...
def find_dup(arr)
  arr.each do |element|
    return num if arr.count(element) == 2
  end
end
